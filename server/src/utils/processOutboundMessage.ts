...
import type { SendInput } from '../modules/internal/v1/messages/send/send.schema.js';
...
export async function processOutboundMessage(
  ...
  if (!external) throw new Error('Failed to create idempotency key');

  const msg: Message = { ...input, externalId: external.key };
